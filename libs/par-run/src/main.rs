use par_run::operations::{Value, begin, case, chan, drain, func_once, link};

#[tokio::main(flavor = "multi_thread")]
async fn main() {
    println!("start");
    // Init
    let (main_s, main_r) = chan();

    // Program
    let (result_s, result_r) = chan();

    let (list_builder_s, list_builder_r) = func_once((), async move |result_s, func_r, _| {
        let (append_s, append_r) = func_once((), async move |result_s, func_r, _| {
                ...
                input_r = case(
                    ...
                    async move |label, input_r, (result_s, mut append_s, mut append_r)| {
                        match label.as_str() {
                            ...
                            "build" => {
                                let (_, inner_r) = func_once((), async move |result_s, _, _| {
    ...
    let print_boxed = || {
        func_once((), async move |result_s, func_r, _| {
            let x = func_r.recv().await.unwrap();
            println!("-> {:?}", x);
            result_s.send(Value::Unit).await.unwrap();
        })

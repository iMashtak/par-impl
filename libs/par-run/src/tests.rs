// use crate::operations::{begin, case, chan, drain, func_once, link, Value};

// #[tokio::test]
// async fn test_list_builder() {
//     println!("start");
//     // Init
//     let (main_s, mut main_r) = chan();

//     // Program
//     let (result_s, result_r) = chan();

//     let (list_builder_s, list_builder_r) =
//         func_once((), async move |result_s, mut func_r, _| {
//             let (append_s, append_r) = func_once((), async move |result_s, mut func_r, _| {
//                 let x = func_r.recv().await.unwrap();
//                 link(result_s, x.into_receiver());
//             });
//             begin(
//                 func_r,
//                 (result_s, append_s, append_r),
//                 async move |loop_s, mut input_r, (result_s, append_s, append_r)| {
//                     input_r = case(
//                         input_r,
//                         (result_s, append_s, append_r),
//                         async move |label, mut input_r, (result_s, mut append_s, mut append_r)| {
//                             match label.as_str() {
//                                 "add" => {
//                                     let x = input_r.recv().await.unwrap();
//                                     (append_s, append_r) = func_once(
//                                     (append_s, append_r, x),
//                                     async move |result_s, mut func_r, (append_s, append_r, x)| {
//                                         let xs = func_r.recv().await.unwrap();
//                                         let (_, inner_r) = func_once(
//                                             (x, xs),
//                                             async move |result_s, _, (x, xs)| {
//                                                 result_s.send(Value::label("item")).await.unwrap();
//                                                 result_s.send(x).await.unwrap();
//                                                 link(result_s, xs.into_receiver());
//                                             },
//                                         );
//                                         append_s.send(Value::Receiver { r: inner_r }).await.unwrap();
//                                         link(result_s, append_r);
//                                     },
//                                 );
//                                     loop_s.send(Some((result_s, append_s, append_r))).unwrap();
//                                 }
//                                 "build" => {
//                                     let (_, inner_r) =
//                                         func_once((), async move |result_s, _, _| {
//                                             result_s.send(Value::label("end")).await.unwrap();
//                                             result_s.send(Value::Unit).await.unwrap();
//                                         });
//                                     append_s.send(Value::Receiver { r: inner_r }).await.unwrap();
//                                     link(result_s, append_r);
//                                     loop_s.send(None).unwrap();
//                                 }
//                                 _ => unreachable!(),
//                             }
//                             input_r
//                         },
//                     )
//                     .await;
//                     input_r
//                 },
//             )
//             .await;
//         });

//     println!("before list_builder_s usage");
//     for i in 0..10 {
//         list_builder_s.send(Value::label("add")).await.unwrap();
//         list_builder_s
//             .send(Value::Str {
//                 x: i.to_string().into(),
//             })
//             .await
//             .unwrap();
//     }
//     list_builder_s.send(Value::label("build")).await.unwrap();

//     let print_boxed = || {
//         func_once((), async move |result_s, mut func_r, _| {
//             let x = func_r.recv().await.unwrap();
//             println!("-> {:?}", x);
//             result_s.send(Value::Unit).await.unwrap();
//         })
//     };

//     println!("before print begin");
//     begin(
//         list_builder_r,
//         (print_boxed,),
//         async move |loop_s, input_r, (print_boxed,)| {
//             let input_r = case(
//                 input_r,
//                 (print_boxed,),
//                 async move |label, mut input_r, (print_boxed,)| {
//                     match label.as_str() {
//                         "item" => {
//                             let x = input_r.recv().await.unwrap();
//                             let (print_s, print_r) = print_boxed();
//                             print_s.send(x).await.unwrap();
//                             if let Err(_) = loop_s.send(Some((print_boxed,))) {
//                                 unreachable!()
//                             }
//                             drain(print_r).await;
//                         }
//                         "end" => {
//                             let _ = input_r.recv().await.unwrap();
//                             if let Err(_) = loop_s.send(None) {
//                                 unreachable!()
//                             }
//                         }
//                         _ => unreachable!(),
//                     }
//                     input_r
//                 },
//             )
//             .await;
//             input_r
//         },
//     )
//     .await;

//     result_s.send(Value::Unit).await.unwrap();

//     link(main_s, result_r);

//     // End
//     println!("before main");
//     let result = main_r.recv().await;
//     if let Some(value) = result {
//         println!("main exit: {:?}", value);
//     } else {
//         panic!("failed")
//     }
// }